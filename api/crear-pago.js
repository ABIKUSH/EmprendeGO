export default async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type');
  if (req.method === 'OPTIONS') return res.status(200).end();
  if (req.method !== 'POST') return res.status(405).json({ error: 'Solo POST' });

  const { email, proveedorId } = req.body || {};
  if (!email || !proveedorId) {
    return res.status(400).json({ error: 'Faltan email o proveedorId' });
  }

  try {
    const response = await fetch('https://api.mercadopago.com/checkout/preferences', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${process.env.MP_ACCESS_TOKEN}`
      },
      body: JSON.stringify({
        items: [{
          title: 'EmprendeGo Plan Pro - 1 mes',
          description: 'WhatsApp directo, badge verificado, productos ilimitados, estadísticas',
          quantity: 1,
          currency_id: 'ARS',
          unit_price: 29
        }],
        payer: { email },
        metadata: { proveedor_id: proveedorId, email },
        back_urls: {
          success: 'https://emprende-go.vercel.app/?pago=ok',
          failure: 'https://emprende-go.vercel.app/?pago=error',
          pending: 'https://emprende-go.vercel.app/?pago=pendiente'
        },
        auto_return: 'approved',
        notification_url: 'https://emprende-go.vercel.app/api/webhook-mp'
      })
    });

    const data = await response.json();

    if (!response.ok) {
      return res.status(500).json({ error: 'Error de Mercado Pago', detail: data });
    }

    return res.status(200).json({
      init_point: data.init_point,
      sandbox_init_point: data.sandbox_init_point
    });
  } catch (err) {
    return res.status(500).json({ error: 'Error interno', detail: err.message });
  }
}
